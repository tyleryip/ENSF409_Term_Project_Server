...
public class RegistrationAppClient {
	...
	public static void main(String[] args) {
		...
		// Note that none of the code in the menu options actually has logic for the
		// course catalogue and other classes.
		// Any logic in the options only deals with taking proper input from the user
		// and then takes the input and calls other back end methods.
		int menu = 0;
		int selection = 0;

		do {
			// Signing in to an existing student name will utilize the existing student
			// object's registrations
			...
			case 1:
				...
				do {
					...
					case 1:
						...
						scan.nextLine();
						System.out.println(
								"Please enter the name and number of the course you would like to search for (ex. ENGG 233)");
					...
					case 5:
						...
						scan.nextLine();
						System.out
								.println("Please enter the name of the course you would like to remove (ex. ENGG 233)");
			...
			case 2: // Administrator sign on
				...
				do {
					...
					System.out.println(
							"6 - Start semester (try to run all courses, if less than 8 students, will cancel that course)");
					...
					case 2:
						...
						scan.nextLine();
						System.out.println(
								"Please enter the name and number of the course you would like to add: (ex. ENGG 233)");
						input = scan.nextLine();
						...
						for (int i = 0; i < inputNum; i++) {
							scan.nextLine();
							System.out.println(
									"Please enter the name and number of the course you would like to add as a prequisite: (ex. ENGG 233)");
					...
					case 3:
						...
						scan.nextLine();
						System.out.println(
								"Please enter the name and number of the course you would like to remove: (ex. ENGG 233)");
						input = scan.nextLine();
					...
					case 6:
						// When testing this function, by default PHYS 259 section 1 will be the only
						// class that will run as it is the only class with 8 students
						for (Course c : cat.getCourseList()) {
							for (CourseOffering o : c.getOfferingList()) {
								if (o.getOfferingRegList().size() < 8) {
									System.out.println(
											c.getCourseName() + " " + c.getCourseNum() + " section " + o.getSecNum()
													+ " has less than 8 people, the section will be cancelled.");
								} else {
									System.out.println(c.getCourseName() + " " + c.getCourseNum() + " section "
											+ o.getSecNum() + " has started classes successfully.");
								}
							}
						}
						pressEnter();
